...
fn main() {
    // Unit sphere with 3D checker pattern (alternating cubes).
    let mut checkers = CheckersPattern::new(
        Color::new(0.9, 0.9, 0.9), // light
        Color::new(0.1, 0.1, 0.1), // dark
    );
    // Slight scale so checkers are large and readable on the sphere.
    checkers.set_transform(&scaling(0.2, 0.2, 0.2) * &rotation_x(-std::f64::consts::FRAC_PI_3));
    ...
    sphere.data.material.pattern = Some(Box::new(checkers));
    ...
    std::fs::write("media/images_ppm/checker_sphere.ppm", ppm)
        .expect("Failed to write checker_sphere.ppm");
    println!("Saved to media/images_ppm/checker_sphere.ppm");
}
