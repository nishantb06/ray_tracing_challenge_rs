// Floor, spheres, and a ceiling plane. Light is placed below the ceiling.
...
fn main() {
    ...
    // Ceiling: default plane has y=0; translate up so ceiling is at y = 5
    let mut ceiling = Plane::new();
    ceiling.set_transform(translation(0.0, 5.0, 0.0));
    ceiling.data.material.color = Color::new(0.95, 0.95, 1.0);
    ceiling.data.material.specular = 0.0;
    ...
    // Light below the ceiling (y=5): put at y=3.5
    world.lights = vec![PointLight::new(
        Tuple::point(-10.0, 3.5, -10.0),
    ...
    std::fs::write("media/images_ppm/ceiling.ppm", ppm).expect("Failed to write ceiling.ppm");
    println!("Saved to media/images_ppm/ceiling.ppm");
}
